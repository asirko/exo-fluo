        ...
        function addToList(recipe, currentServings) {
            recipe.ingredients.forEach(function (ingredient) {
                ...
                if (ingredientInList.unit && ingredient.unit !== ingredientInList.unit) {
                    logger.error('The unit of ' + ingredient.name + ' isn\'t corresponding, ' +
                        'it hadn\'t been added to the list.');
                } else {
                    ingredientInList.unit = ingredient.unit;
                    ingredientInList.quantity = (ingredientInList.quantity || 0) +
                        currentServings / recipe.servings * ingredient.quantity;
                }
            });
            return $q.when();
        }
